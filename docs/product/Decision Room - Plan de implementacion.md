...
**Siguiente trabajo:** una vez comprobada esta ampliación, paso 1.7, variedad de casos sobre el recorrido integrado. Repetir DR-001 y DR-002, variar columnas y contextos, probar correcciones y preguntas con los modelos reales y comparar otro modelo si persisten errores. Las pruebas del controlador y algunos resultados revisados no cierran la evaluación de calidad del agente.

La entrega 1 completa todavía no está aceptada: ya existe el recorrido interno desde la ingesta hasta un informe revisado, pero falta superar la variedad de casos y la evaluación semántica prevista en el paso 1.7. Las pruebas realizadas no equivalen a validación general del producto.
