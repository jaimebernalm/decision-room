...
def chart_html(data, chart):
    ...
    points = chart['points']
    values = [Decimal(str(metric(data, p['value']))) for p in points]
    ...
    if chart['kind'] != 'table':
        ...
        svg = f'<svg viewBox="0 0 720 {height}" role="img" aria-labelledby="{title_id} {caption_id}">' + ''.join(shapes) + '</svg>'
        table = '<details><summary>Ver los valores del gráfico</summary>' + table + '</details>'
    return (f'<figure><h3 id="{title_id}">{e(chart["title"])}</h3><p class="unit">{e(chart["unit"])}</p>' + svg +
            f'<figcaption id="{caption_id}">{e(chart["caption"])}</figcaption>' + table + '</figure>')
...
def evidence_html(data, claim, charts):
    ...
    return ('<details class="evidence"><summary>Ver cómo se ha calculado</summary>'
            ...
            '<th scope="col">Valor guardado</th></tr></thead><tbody>' + ''.join(rows) + '</tbody></table></details>')
...
@media(max-width:640px){main{margin:0;border:0;border-radius:0}header{padding:30px 22px}.content{padding:24px 22px}footer{padding:22px}figure{padding:12px}.chart-label,.chart-number{font-size:17px}}
...
def render_client(data, exported_at, *, embedded=False):
    ...
    if ready:
        ...
        for i, claim in enumerate(draft['claims'], 1):
            ...
            body += [f'<section class="finding"><p class="number">HALLAZGO {i:02d}</p><h2>{e(claim["title"])}</h2><p>{e(claim["statement"])}</p>']
            body += [chart_html(data, c) for c in charts]
