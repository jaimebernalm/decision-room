...
def export(config, business_id, review_id):
    ...
    with session_lock(config, business_id, review._parent(config, business_id, review_id)) as (db, _), db.transaction():
        memory_lock(db, business_id)
        data = review.show(config, business_id, review_id, _db=db)
